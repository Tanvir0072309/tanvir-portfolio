...
const slidesData = [
  { img: p1, text: "1. AEKYAM 2.O PARTICIPATION ( 1 )" },
  { img: p2, text: "2. TECH DYNESTY BEAUTY WITH BRAIN PARTICIPATION" },
  { img: p3, text: "3. DANCE COMPITETION 2024 PARTICIPATION" },
  { img: p4, text: "4. AEKYAM 2.O WEB DESIGN PARTICIPATION" },
  { img: p5, text: "5. AEKYAM 2.O PARTICIPATION ( 2 )" },
  { img: p6, text: "6. TECH DYNESTY ANDHADHUN PARTICIPATION" },
  { img: p7, text: "7. ENIGMA 2K24 PARTICIPATION" },
  { img: p8, text: "TEACHER'S DAY PARTICIPATION" },
]
...
export default function Participation() {
  ...
  const prev = () =>
    setActive((active - 1 + slidesData.length) % slidesData.length)

  const next = () =>
    setActive((active + 1) % slidesData.length)
    ...
    <section id="certs">
      ...
      <div className="slider-box">
        {slidesData.map((slide, i) => {
          ...
          else if (i === (active + 1) % slidesData.length) className += " next"
          else if (i === (active - 1 + slidesData.length) % slidesData.length)
            className += " prev"
      ...
      <div style={{ textAlign: "center", marginTop: "40px" }}>
        <button onClick={prev} className="btn-main">PREV</button>
        <button onClick={next} className="btn-main" style={{ marginLeft: "15px" }}>
          NEXT
        </button>
      </div>
    </section>
